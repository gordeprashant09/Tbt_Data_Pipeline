...
def _merge_one_symbol_streaming(
    ...
    # Build per-instrument pyarrow token sets once (same accurate logic as old script)
    inst_pa_sets = {
    ...
    for shard in shards:
        ...
        del table, token_col
        _trim_malloc()  # thread-safe; gc.collect() only in parent after all threads
...
def _merge_one_expiry_symbol(
    ...
    gc.collect()  # once per expiry, after all threads finish
...
def merge_expiry_shards(
    ...
    for i, expiry_dir in enumerate(expiry_dirs_sorted, 1):
        ...
        _trim_malloc()  # gc.collect() already called inside _merge_one_expiry_symbol

    return final_files


# ---------------------------------------------------------------------------
# Sanity checks — same signature as old + saves CSV with cross-check section
# ---------------------------------------------------------------------------
def run_sanity_checks(final_files: dict, row_count_csv: Path, output_root: Path, date: str) -> bool:
    ...
    # per-file results stored for CSV
    csv_rows = []
    ...
    # Save sanity CSV directly in output_root
...
def main():
    ...
    if passed:
        ...
        print(f"          +-- FUTSTK/")
        print(f"          |   +-- RELIANCE/")
        print(f"          |       +-- {date}_FUTSTK_RELIANCE_1459348200.parquet")
        print(f"          +-- OPTSTK/")
        print(f"          +-- FUTIDX/")
        print(f"          +-- OPTIDX/")
        print(f"          +-- support_docs/")
        print(f"          +-- sanity_check_{date}.csv  (in logs dir)")
    else:
