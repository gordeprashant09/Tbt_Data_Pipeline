...
def tar_and_publish(final_dir: Path, output_root: Path, date: str):
    ...
    Why tar (no compression):
      ...
      - Plain tar bundles 1268+ files into 1 → ~2x faster NAS transfer
      - Easy to extract: tar -xf <date>_parsed.tar
    """
...
def main():
    ...
    local_work_dir = Path(f"/home/alpha/allseg_{date}_work")
    ...
    # ── TAR bundle — single file upload to local + NAS (~2x faster than file-by-file)
    # Runs after existing publish; existing logic unchanged
    tar_and_publish(dirs["final"], local_work_dir, date)
