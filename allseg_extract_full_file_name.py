...
Usage:
    /home/alpha/myenv/bin/python -u allseg_date_inst_symbol.py --date 20260320
    /home/alpha/myenv/bin/python -u allseg_date_inst_symbol.py --date 20260320 --clean
"""
...
# NAS output path
PARSED_DATA_NAS  = Path("/mnt/historical_data/parsed_data")
...
# Discover archives
# ---------------------------------------------------------------------------
def select_efvi_cap(source_root: Path) -> "Path | None":
    """
    Select the best efvi .cap file (not the tar.xz archive) in source_root.

    Priority:
      1. Both recovery_efvi_*.cap AND stitched_efvi_*.cap exist
         → Use recovery if its size >= stitched, otherwise use stitched
      2. Only stitched_efvi_*.cap exists  → use stitched
      3. Only recovery_efvi_*.cap exists  → use recovery
      4. Neither exists                   → return None (caller skips efvi)

    Prints a one-line explanation of the choice.
    """
    recovery_files = sorted(source_root.glob("recovery_efvi_*.cap"))
    stitched_files = sorted(source_root.glob("stitched_efvi_*.cap"))

    recovery = recovery_files[0] if recovery_files else None
    stitched = stitched_files[0] if stitched_files else None

    if recovery and stitched:
        r_sz = recovery.stat().st_size
        s_sz = stitched.stat().st_size
        if r_sz >= s_sz:
            print(f"[EFVI] Using recovery_efvi  ({r_sz / 1e9:.2f} GB) "
                  f">= stitched_efvi ({s_sz / 1e9:.2f} GB)")
            return recovery
        else:
            print(f"[EFVI] recovery_efvi too small ({r_sz / 1e9:.2f} GB) "
                  f"< stitched_efvi ({s_sz / 1e9:.2f} GB) → using stitched_efvi")
            return stitched
    elif stitched:
        print(f"[EFVI] No recovery_efvi found — using stitched_efvi "
              f"({stitched.stat().st_size / 1e9:.2f} GB)")
        return stitched
    elif recovery:
        print(f"[EFVI] No stitched_efvi found — using recovery_efvi "
              f"({recovery.stat().st_size / 1e9:.2f} GB)")
        return recovery
    else:
        return None
...
def _process_one_archive_worker(args):
    ...
    # Plain .cap file (e.g. recovery_efvi / stitched_efvi) — no extraction needed
    if src.suffix == ".cap":
    ...
    # --- Normal path: .cap.tar.xz ---
    # Extract directly from NAS source (no pointless NAS→NAS copy)
...
# ---------------------------------------------------------------------------
def publish_to_parsed_nas(final_files: dict, row_count_csv: Path, date: str):
...
def main():
    ...
    source_root    = Path(f"/mnt/historical_data/{date}")
    ...
    # --- efvi selection: recovery_efvi preferred if size >= stitched_efvi ---
    efvi_cap = select_efvi_cap(source_root)
    if efvi_cap is not None:
        source_archives = list(source_archives) + [efvi_cap]
        print(f"[EFVI] Added to pipeline: {efvi_cap.name}")
    else:
        print(f"[EFVI] No efvi .cap file found in {source_root} — skipping efvi")
    ...
    publish_to_parsed_nas(final_files, row_count_csv, date)
